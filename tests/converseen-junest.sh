...
DEPENDENCES="imagemagick djvulibre libraw openexr libheif jp2a kvantum" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
LIBSAVED="libpxbackend libGLdispatch libcurl libduktape libnghttp libidn libssh libpsl libunistring libuuid libxcb ImageMagick $lib_audio_keywords $lib_browser_launcher"
...
_extract_package() {
	...
	if test -f "$pkg_full_path"; then
		...
		[ -n "$lib_browser_launcher" ] && [[ "$arg" =~ (hicolor-icon-theme|xapp|imagemagick) ]] && tar fx "$pkg_full_path" -C ./base/ --warning=no-unknown-keyword --exclude='.PKGINFO'
	fi
}
