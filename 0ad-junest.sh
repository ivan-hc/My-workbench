...
DEPENDENCES="ca-certificates alsa-lib libpulse" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCES ARE NEEDED
...
function _extract_main_package() {
	mkdir -p base
	rm -R -f ./base/*
	tar fx "$(find ./"$APP".AppDir -name "$APP"-*[0-9]*zst | grep -v data | head -1)" -C ./base/
...
LIBSAVED="pk p11 alsa jack pipewire pulse GL libX $lib_browser_launcher" # Enter here keywords or file/directory names to save in /usr/lib
